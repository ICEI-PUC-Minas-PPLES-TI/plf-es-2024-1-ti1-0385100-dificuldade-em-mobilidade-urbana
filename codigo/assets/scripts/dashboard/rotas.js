...
function atualizarTabela() {
    ...
    tabela.innerHTML = '<tr><th class="pontos">Pontos</th><th>Alertas</th><th>ID</th><th class="texto-acoes">Ações</th></tr>';

    rotas.forEach((rota, index) => {
        const linha = document.createElement('tr');

        console.log(Object.values(rota));
        Object.values(rota).forEach((valor) => {
            const celula = document.createElement('td');
            if (Array.isArray(valor)) {
                if (valor.length === 0) {
                    celula.textContent = "Nenhum alerta";
                } else {
                    celula.textContent = valor.map(ponto => ponto.endereco).join(', ');
                }
            } else {
...
async function criarRota(paradas) {
    ...
    const response = await fetch(`${API_URL}rotas`, {
        ...
        body: JSON.stringify({stops: paradasArray})
    })
